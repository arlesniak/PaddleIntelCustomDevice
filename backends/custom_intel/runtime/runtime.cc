...
C_Status Init() {
  std::cout << "custom_intel plugin compiled with ";
#ifdef __clang__
  std::cout << "clang\n";
#else
  std::cout << "gcc\n";
...
C_Status Allocate(const C_Device device, void **ptr, size_t size) {
  std::cout << "custom_intel plugin Allocate, size=" << size << std::endl;
...
C_Status Deallocate(const C_Device device, void *ptr, size_t size) {
  std::cout << "custom_intel plugin DeAllocate, size=" << size << std::endl;
  free(ptr);
  return C_SUCCESS;
}
...
C_Status CreateStream(const C_Device device, C_Stream *stream) {
  ...
  *stream = reinterpret_cast<C_Stream>(&q);
  return C_SUCCESS;
}
...
void InitPlugin(CustomRuntimeParams *params) {
  ...
  params->device_type = "custom_intel";
