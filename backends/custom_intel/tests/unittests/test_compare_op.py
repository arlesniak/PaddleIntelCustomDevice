...
def create_paddle_case(op_type, callback):
    class PaddleCls(unittest.TestCase):
        def setUp(self):
            ...
            self.place = fluid.CustomPlace('custom_intel', 0)
            if core.is_compiled_with_cuda():
                self.place = paddle.CustomPlace('custom_intel', 0)
        ...
        def test_dynamic_api(self):
            paddle.disable_static(paddle.CustomPlace('custom_intel', 0))
    ...
    def test_errors(self):
        ...
        with program_guard(Program(), Program()):
            ...
            y = fluid.create_lod_tensor(
                numpy.array([[-1]]), [[1]], fluid.CustomPlace('custom_intel', 0))
...
class API_TestElementwise_Equal(unittest.TestCase):
    def test_api(self):
        ...
        with fluid.program_guard(fluid.Program(), fluid.Program()):
            ...
            place = fluid.CustomPlace('custom_intel', 0)
...
class TestCompareOpPlace(unittest.TestCase):
    def test_place_1(self):
        paddle.enable_static()
        place = paddle.CustomPlace('custom_intel', 0)
        if core.is_compiled_with_cuda():
            place = paddle.CustomPlace('custom_intel', 0)
        label = fluid.layers.assign(np.array([3, 3], dtype="int32"))
        limit = fluid.layers.assign(np.array([3, 2], dtype="int32"))
    ...
    def test_place_2(self):
        ...
        if core.is_compiled_with_cuda():
            place = paddle.CustomPlace('custom_intel', 0)
            data_place = paddle.CustomPlace('custom_intel', 0)
