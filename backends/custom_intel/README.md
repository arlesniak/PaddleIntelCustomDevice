...
pip install dist/paddle_custom_intel*.whl
```
...
# Compile the plug-in
mkdir -p PaddleCustomDevice/backends/custom_intel/build
pushd PaddleCustomDevice/backends/custom_intel/build
...
# Modify resnet50_test.cc, use config.EnableCustomDevice("custom_intel", 0) to replace config.EnableUseGpu(100, 0)
